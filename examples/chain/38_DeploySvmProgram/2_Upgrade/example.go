...
func BuildUpgradeMsg(
	...
	data := svm.MustMarshalIxData(svm.Upgrade{})
	...
	tx, err := txBuilder.AddInstruction(upgradeIx).Build()
	if err != nil {
		panic(err)
	}
...
func main() {
	...
	cosmosPrivateKeys := []*ethsecp256k1.PrivKey{
		...
		{Key: ethcommon.Hex2Bytes("6c212553111b370a8ffdc682954495b7b90a73cedab7106323646a4f2c4e668f")},
	}
	...
	// when upgrade, we only know the program pubkey
	programPubkey := solana.MustPublicKeyFromBase58("Eutyt8ETuT9Zsjj8z6hxdwnzaoQadgMzANr25t3DyMk3")

	programBufferSvmPrivKey := ed25519.GenPrivKeyFromSecret([]byte("programBufferV2"))
	programBufferPubkey := solana.PublicKeyFromBytes(programBufferSvmPrivKey.PubKey().Bytes())

	fmt.Println("program buffer v2:", programBufferPubkey.String())

	fmt.Println("linking new buffer account for program v2")
	...
	// upload programs
	fmt.Println("uploading program and finalize")
	...
	signedTx, err = BuildSignedTx(chainClient, []sdk.Msg{upgradeMsg}, cosmosPrivateKeys)
	if err != nil {
		panic(err)
	}
	...
	if err != nil || res.TxResponse.Code != 0 {
		if err == nil {
			err = fmt.Errorf(res.TxResponse.RawLog)
		}
